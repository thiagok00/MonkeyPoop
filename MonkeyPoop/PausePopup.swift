...
class PausePopup: SKSpriteNode {
    
    
    class func createPausePopup(size:CGSize)->PausePopup {
        
        
        let pauseMenu = PausePopup(color: UIColor(red: 53/255, green: 42/255, blue: 42/255, alpha: 1), size: CGSizeMake(size.width*0.8, size.height*0.8))
        ...
        let restartButton = SKSpriteNode(imageNamed: "ResumeButton")
        restartButton.name = "Resume"
